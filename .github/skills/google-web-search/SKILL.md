...
2. **소스(동적, 고정 화이트리스트 아님)**: 주제별 가장 권위 있는 1차 출처를 탐색·검증.
   ...
   - MS/Azure/GitHub 주제: 일반어 대신 구체 서비스명(Microsoft Foundry·Azure AI Search·Azure AI
     Content Safety·Microsoft Entra·Microsoft Purview·Microsoft Defender·GitHub Copilot/GitHub
     Advanced Security)으로 기능·통제·가치 수집.
3. **검색과 원문 수집**:
...
5. **정리**: `## 검색 결과` + 검색일·핵심 쿼리 + 핵심요약(1~3문장) + 상세 +
   `### 출처`(제목·원문 URL·해당 주장의 근거). 변동 가능성이 있으면 확인 시점을 명시한다.
