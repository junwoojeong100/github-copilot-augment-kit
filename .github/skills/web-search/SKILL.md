...
## 실행 전 capability 확인
- 스킬은 검색 전략을 제공할 뿐 검색 backend를 생성하지 않는다. 먼저 현재 client가 제공하는 도구를
  확인한다.
- 범용 최신 검색에는 다음 중 하나가 필요하다: 전용 `web_search`, web source를 지원하는 Research agent,
  또는 조직이 승인한 general search MCP/API.
- Microsoft Learn MCP·GitHub search처럼 범위가 제한된 도구만 있으면 해당 도메인만 최신 검증할 수 있다.
  이를 범용 웹 검색 성공으로 간주하지 않는다.
- 범용 검색 capability가 없고 공식 URL도 모르면 공개 SERP를 직접 조회하지 않는다. 필요한 검색 provider를
  활성화하거나 사용자가 출발 URL을 제공해야 함을 먼저 알리고, 현재 지식만으로 최신 사실을 만들지 않는다.
...
3. **검색과 원문 수집**:
   - 다음 capability 순서로 경로를 선택한다. 앞 경로가 없거나 실패할 때만 다음 경로로 전환한다.
     1. 전용 general web search tool(예: `web_search`)
     2. web source를 지원하는 client의 Research agent 또는 조직이 승인한 search MCP/API
...
## 처리
결과 없음→쿼리 재구성 1회. 검색 provider 자체가 차단되면 다른 공개 SERP를 scraping하는 fallback 대신
도메인 전용 검색 또는 공식 index/sitemap/RSS 경로로 전환한다. 비신뢰 출처만 있으면 사실 확정을 보류한다.
