...
Runner가 audit·전체 render를 병렬 실행하고, risk score가 높은 슬라이드를 같은 PDF에서 상세 렌더한다.
`verification-report.json`, `qa/contact-*.jpg`, `qa-detail/slide-*.jpg`를 확인한 뒤 결함을 일괄 수정한다.
`--strict`는 작은 본문 후보와 title risk를 실패 처리한다. 사람이 확인한 의도적 보조 텍스트가 있을
때만 `--allow-small-text`를 사용한다.
