...
`python-pptx`로 직접 구성한다. 가장 중요한 표현 기준은 **Straightforward**다. 청중이 첫 장면부터
결론·의미·다음 행동을 이해하도록 만들되, 제작과 검증에 드는 시간은 최소화한다.
...
## Straightforward 슬라이드 원칙

- **답을 먼저 보여준다**: 표지가 있으면 첫 본문 슬라이드, 표지가 없으면 첫 슬라이드에서 핵심 결론,
  추천안 또는 학습 목표와 다음 행동을 명확히 제시한다.
- **한 장 한 질문·한 결론**: 제목은 주제가 아니라 청중이 받아들일 결론으로 쓰고, 근거는 그 결론을
  지지하는 2~4개만 남긴다.
- **가장 짧은 읽기 경로**: 좌→우 또는 상→하로 한 번에 읽히는 구조를 우선하고, 장식·중복 요약·
  불필요한 agenda·제품 나열·여러 겹의 카드 안 카드를 제거한다.
- **복잡함을 숨기되 사실을 숨기지 않는다**: 기술적 복잡성은 익숙한 언어와 단계로 정리하지만,
  의사결정에 필요한 근거·위험·조건·출처는 유지한다.
- 지정 장수를 맞추기 위한 filler를 만들지 않는다. 내용이 부족하면 반복 설명 대신 사례·근거·비교·
  결정 기준으로 보강한다.

## 핵심 원칙

- **Straightforward가 최우선**: 자유로운 시각화와 높은 완성도는 결론 전달을 돕기 위한 수단이다.
  새로워 보이는 구성보다 청중이 빠르게 이해하고 결정할 수 있는 구성을 선택한다.
...
1. 외부 사실이 있는 발표자료는 매 요청마다 `web-search`와 공식 도구로 실시간 조사한다. 먼저
   general search capability를 확인하며, 없으면 공개 SERP를 직접 조회하지 말고 provider 활성화 또는
...
- 결론형 제목
- 청중이 기억해야 할 한 문장
- 이 장을 본 뒤 청중이 해야 할 판단·행동
- 근거와 출처(어떤 Fact Ledger 항목을 쓰는지)
- 정보 관계(숫자/흐름/비교/계층/사례 등)에 맞는 시각 형태
- 이전/다음 슬라이드와의 논리적 연결

조사와 스토리라인이 잠기기 전에는 슬라이드 코드를 작성하지 않는다. 변경이 필요하면 먼저
`storyline.md`를 갱신한다. 결론이나 판단을 바꾸지 않는 장은 제거·통합하고, 요청 장수가 고정이면
반복 문구 대신 근거·사례·비교·실행 기준으로 교체한다.
...
- 같은 구조를 기계적으로 반복하지 않는다. 숫자는 크게, 과정은 흐름으로, 비교는 공통 축으로, 구조는
  계층으로 표현한다.
- 여러 시각 구조가 가능하면 **설명 없이 가장 빨리 읽히는 형태**를 선택한다. 복합 도식은 단순화하고,
  상세 아키텍처가 필요할 때만 단계·경계·책임을 추가한다.
...
- 요청한 장수와 파일 형식이 정확하다.
- 표지와 첫 본문 슬라이드만 보아도 발표의 결론·가치·다음 행동이 분명하다.
- 제목만 읽어도 서사가 이어진다.
...
| 실패 | 교정 |
|---|---|
| 멋있지만 무엇을 말하는지 늦게 드러남 | 첫 본문에 결론·가치·다음 행동을 배치하고 이후 장은 근거 순서로 재구성 |
| 한 장에 여러 메시지와 중첩 카드가 있음 | 한 질문·한 결론만 남기고 가장 짧은 읽기 경로의 표·흐름·비교로 단순화 |
