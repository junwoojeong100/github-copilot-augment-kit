...
- 독립적인 조사는 research agent나 `/fleet`을 사용하지 않고 공식 검색·문서 도구의 병렬 tool call로
  수행하며, 메인 에이전트가 Fact Ledger를 합친다.
...
이 결과를 세션 작업 폴더의 `deck-spec.md`에 확정한다. 조사 단계와 생성 단계를 분리하며,
변경이 필요하면 먼저 deck spec을 갱신한다.
