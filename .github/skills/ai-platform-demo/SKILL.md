...
`customer-overlay.json`을 작성한다. Overlay는 실시간 research metadata, 고객 meta·story·전체
Design DNA, 핵심 route와 Agent를 소유한다.

### 4단계 — 빌드
1. `reference/golden-runtime.md`, `reference/adaptive-design-dna.md`,
   `reference/composable-spec.md`, `reference/demo-spec.md`를 읽는다.
2. 고객 산업과 가까운 `packs/*.pack.json`을 선택한다. Pack은 산업 terminology·공식·Agent 역할의
   출발점만 제공하며 `meta`, `story`, `design`을 포함할 수 없다.
3. 실시간 조사 결과와 고객 Design DNA를 `customer-overlay.json`에 작성한다. Pack이 요구하는
...
4. Composer로 base + Industry Pack + Customer Overlay를 합치고 단일 HTML까지 한 번에 생성한다.
   ...
   ```
5. Composer는 Overlay에 전체 Design DNA가 없거나 research metadata가 오래됐거나 Pack의 고객
   필수 path가 빠지면 실패해야 한다. Repository example/test에서만 `--allow-stale-research`를 쓴다.
6. 기본 수정 surface는 HTML이나 전체 Spec이 아니라 `customer-overlay.json`이다. 고객 콘텐츠·색상·
   공식·Agent를 바꾸면 Overlay를 수정하고 재합성한다.
7. 적합한 Pack이 없으면 기존 방식대로 전체 `demo-spec.json`을 새로 작성하고 `render_demo.py`를
   직접 사용한다. 속도를 위해 맞지 않는 Pack을 억지로 사용하지 않는다.
...
9. 화면별 코드를 여러 에이전트에 분산하지 않고 메인 에이전트가 Design DNA·Overlay·최종 HTML을 일관되게 소유한다.
...
- `scripts/compose_demo_spec.py` — layer merge·live research·Design DNA gate·Spec/HTML 동시 생성.
