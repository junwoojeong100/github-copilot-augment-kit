...
`demo-spec.json`은 고객별 콘텐츠·산업 동작을 한 곳에 잠그는 build contract다(디자인은 고정 Microsoft 톤). Fact
...
모든 top-level section은 필수다. `design`은 base spec이 고정 Microsoft-톤으로 제공하며 고객 Overlay는 정의하지 않는다.
...
3. 디자인은 Microsoft 톤으로 고정되어 base가 제공한다(고객 Overlay에는 `design`을 넣지 않는다).
