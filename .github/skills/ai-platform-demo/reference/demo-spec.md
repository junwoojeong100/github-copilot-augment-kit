...
ID와 순서는 고정이지만 메뉴명·crumb는 `DEMO_FOCUS`와 고객 업무에 맞게 바꾼다. 메뉴·제목·버튼·상태에
장식용 영어를 쓰지 않는다. `GitHub Copilot`, `Microsoft Foundry`, `AKS`, `OEE`, `ESS`, `PR`처럼
공식명 또는 일반 약어인 경우는 예외다.
...
- 8개 route는 모두 완성하지만 `story.climax`와 핵심 4~6개 장면은 `DEMO_FOCUS`에 맞춰 정한다.
