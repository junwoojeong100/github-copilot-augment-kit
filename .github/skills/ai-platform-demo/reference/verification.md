...
완료 전에는 `FULL_QA=1`로 다시 실행한다. 이 모드에서는 `VERIFY_ROUTES`가 설정돼 있어도
`story.routeScope`의 모든 route를 검사하므로 최종 gate가 실수로 축소되지 않는다. `SHOTS_DIR`는
필수이며 session 작업 폴더 밖을 가리키면 verifier가 거부한다.
