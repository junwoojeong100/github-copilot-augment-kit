...
> 📌 **Azure vs AWS/GCP**, **GitHub vs GitLab** 등 경쟁사 간 비교는 `cloud-competitive-analysis` 스킬을 사용하세요. 이 모드는 Azure 서비스 간 선택에 집중합니다.
...
| Managed Identity 안됨 | 시스템/사용자 할당 확인, 역할 전파 시간 (보통 수 분, 최대 ~30분) |
...
| 복잡한 가이드 | 인라인 제공 (사용자가 요청한 경우에만 `<service>-guide.md` 파일 생성) |
