...
| 범용 최신 웹 검색 | client의 web search 도구 또는 별도 MCP 필요 | 내장 Research agent/web 도구 또는 별도 MCP 사용 |
...
| 스킬 | 트리거 예시 | 기능 |
|------|-----------|------|
| **web-search** | "최신 버전 알려줘", "최근 업데이트" | 전용 검색 도구·공식 문서 검색을 사용하고 공개 SERP scraping 없이 원문을 검증한 뒤 결론부터 답변 |
...
외부 사실 조사는 캐시로 생략하거나 research agent·`/fleet`에 위임하지 않습니다. 이전 Fact Ledger와
URL은 검색 출발점으로만 사용하며, 기능 상태·가격·규제·고객 성과는 발표 요청마다 현재 공식 원문으로
다시 확인합니다. 서로 독립적인 조사 축은 메인 에이전트가 공식 검색·문서 도구의 병렬 tool call로
직접 수행하고 하나의 Fact Ledger로 합칩니다.
...
이 킷의 `web-search` 스킬은 **검색 전략·검증·출력 형식**을 맡으며 검색 backend 자체를 제공하지
않습니다. 실제 검색 capability는 client의 web search/Research 도구 또는 MCP/API가 제공합니다.
MCP(Model Context Protocol) 서버는 Copilot에 **구조화된 외부 도구 접근**
(공식 문서·GitHub 이슈/PR/코드·선택한 검색 provider)을 붙여 줍니다.
...
| **General search provider** | 뉴스·기업·산업 등 범용 최신 웹 검색 | provider별 상이 | **번들하지 않음** — client 내장 도구 또는 신뢰할 수 있는 MCP/API 사용 |
...
> GitHub·Microsoft Learn 범위에는 별도 서버 설치가 필요하지 않습니다. 범용 최신 웹 검색은 client가
> web search/Research capability를 제공하지 않는 경우 별도의 신뢰할 수 있는 search MCP/API가 필요합니다.
...
검색은 client의 web search/Research capability를 사용하거나, 없는 환경에서는 `/mcp add` 또는
`.vscode/mcp.json`으로 신뢰할 수 있는 general search provider를 별도로 연결해야 합니다.
