...
스킬이 자동 로드되어 ① 고객·산업 리서치 → ② 산업별 8화면 설계 → ③ 단일 HTML 빌드 → ④ puppeteer로 렌더링·에러 검증까지 수행합니다. 독립 조사는 병렬화하되 최종 HTML은 한 에이전트가 조립하며, Puppeteer·Chromium은 저장소 밖 공용 캐시에서 재사용합니다. 검증 스크립트·스크린샷은 세션 작업 폴더에 격리하고 저장소에는 최종 HTML 하나만 남깁니다.
...
그 후 ① 공식 자료 조사·팩트 원장 → ② 목적별 스토리라인 → ③ 편집 가능한 PPTX 생성 →
④ LibreOffice·PyMuPDF 전체 렌더 및 contact sheet 검수 순으로 진행합니다. 재생성 Python 스크립트와
QA 파일은 세션 작업 폴더에 격리하며 저장소에는 최종 PPTX만 남깁니다. 독립 조사는 병렬화하고,
동일 PPTX 리비전의 중간 PDF는 SHA-256 검증 후 상세 슬라이드 렌더에 재사용합니다. 수정 후에는
변경 부분을 먼저 확인하되 완료 전 전체 구조 감사와 전체 렌더를 다시 수행합니다.
...
두 생성 스킬은 기본적으로 **FULL-OPTIMIZED** 정책을 사용합니다. 모든 조사·스토리라인·디자인·제작·
전체 QA 단계를 유지하면서 공용 도구 캐시, 제한적 병렬 조사, 결함 일괄 수정, 증분 회귀 확인,
최종 전체 검증으로 중복 실행을 줄입니다.
