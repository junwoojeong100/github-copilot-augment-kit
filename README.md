...
└── skills/                              # 전문 스킬 (온디맨드)
    ...
    │   ├── scripts/                     # Spec→HTML Renderer·브라우저 검증기
    │   ├── examples/                    # 고객 콘텐츠를 복제하지 않는 구조 예제
    │   └── reference/                   # Design DNA·화면 청사진·Runtime·검증 절차
    └── adaptive-presentation/           # 주제·청중별 고품질 PPTX 생성기
        ├── SKILL.md                     # 조사→서사→Design DNA→PPTX→렌더 QA
        ├── pptx_compiler/               # theme-agnostic 슬라이드 생성·preflight 엔진
        ├── examples/                    # 동일 semantic deck의 Design DNA 변형 예제
        ├── reference/                   # 서사·Design DNA·청사진·Compiler 사용법
...
| **ai-platform-demo** | "○○ 고객 △△ 산업 AI 데모 만들어줘", "임원 데모", "운영 대시보드 데모" | 고객별 `demo-spec.json` + Golden Runtime → 실제 동작하는 단일 HTML SaaS 데모(8화면) 생성·전체 QA |
...
스킬이 자동 로드되어 ① 고객·산업 리서치 → ② Storyline·Adaptive Design DNA·8개 화면 계약 →
③ `demo-spec.json` 작성 → ④ Golden Runtime Renderer로 단일 HTML 생성 → ⑤ Puppeteer 전체 QA까지
수행합니다.

```text
Fact Ledger + Storyline + Design DNA + demo-spec.json
...
**고객·산업이 바뀌어도 재현** — `demo-spec.json`에서 palette role, 도메인 화면, 에이전트, KPI,
시뮬레이터 공식을 바꾸고, Golden Runtime의 route lifecycle·interaction engine·QA hook은
검증된 상태로 유지합니다. 내장 산업 매핑표는 레미콘·은행·유통·물류·제조·에너지·헬스케어 등의
출발점을 제공하지만 실제 고객 사실과 용어로 다시 작성합니다.
...
그 후 ① 공식 자료 조사·Fact Ledger → ② 목적별 Storyline과 slide blueprint → ③ Design DNA를
입력받는 `pptx_compiler`로 편집 가능한 PPTX 생성 → ④ LibreOffice·PyMuPDF 전체 렌더 및 contact
sheet 검수 순으로 진행합니다.

```text
Fact Ledger + Deck Spec + Design DNA + Semantic Blueprints
...
python3 -B .github/skills/adaptive-presentation/scripts/render_pptx.py \
  deck.pptx --out /tmp/deck-qa --keep-pdf
python3 -B .github/skills/adaptive-presentation/scripts/render_pptx.py \
  deck.pptx --reuse-pdf /tmp/deck-qa/deck.pdf --slides 8,16 \
  --keep-slide-images --out /tmp/deck-detail
```

렌더 기본값은 **최대 30장을 한 장에 담은 소형 JPEG overview**이며, 개별 슬라이드 이미지는 삭제됩니다.
중간 PDF는 동일 PPTX 리비전의 상세 검사 동안만 유지하고 QA 종료 시 삭제합니다. PPTX/PDF를 대화에
직접 첨부하지 않고 의심 슬라이드만 선택 렌더해 첨부 용량 초과를 방지합니다.
...
|---|---|---|
| **생성 메커니즘 재사용** | Golden Runtime의 SPA lifecycle·interaction·QA hook 재사용 | Slide Compiler의 primitive·named region·preflight 재사용 |
| **요청별 변경 surface 축소** | 고객별 사실·Design DNA·화면 계약을 `demo-spec.json`에서 수정 후 재생성 | 덱별 Design DNA·콘텐츠·semantic blueprint만 생성 스크립트에 작성 |
...
| **수정 루프 단축** | 결함을 모아 일괄 수정 → 영향 route 확인 → 최종 전체 QA | 결함을 모아 일괄 수정 → 변경 슬라이드 확인 → 최종 전체 audit/render |
