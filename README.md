...
이 킷의 `google-web-search` 스킬이 **범용 웹 검색**을 맡는다면,
**MCP(Model Context Protocol) 서버**는 Copilot에 **구조화된 1차 도구 접근**
(공식 문서·GitHub 이슈/PR/코드)을 붙여 줍니다. 둘은 상호 보완적입니다 — 스킬은
"어떻게 정리할지", MCP는 "어디서 정확히 가져올지".
